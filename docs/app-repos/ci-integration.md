...
App repos use a single GitHub Actions workflow (`apx-publish.yml`) that triggers on tag push. It validates schemas locally and publishes to the canonical repository via a pull request.
...
The workflow file must exist at `.github/workflows/apx-publish.yml`. It is generated automatically during initialization or can be regenerated:
...
| **Parse tag** | Extracts the tag from `GITHUB_REF` (e.g. `proto/payments/ledger/v1/v1.0.0`) and passes it to the publish step. |
...
| **Publish** | `apx publish` clones the canonical repo, creates a release branch, copies schema files, generates `go.mod`, and opens a PR. |
...
- If `.github/workflows/apx-publish.yml` exists → app repo → regenerates `apx-publish.yml`
...
```yaml
      ...
      # Add your custom steps between validate and publish
      - name: Run tests
        run: |
          apx gen go && apx sync
          go test ./...
...
```yaml
      ...
      - name: Notify Slack
        ...
        with:
          ...
          payload: |
            {"text": "Published ${{ steps.tag.outputs.tag }} to canonical repo"}
```
...
ls .github/workflows/apx-publish.yml

# Dry-run a publish to check connectivity
apx publish proto/payments/ledger/v1 --version v1.0.0-alpha.1 --dry-run
```

### Checklist

- [ ] `apx.yaml` has correct `org`, `source.repo`, and `api.id`
- [ ] `.github/workflows/apx-publish.yml` exists with correct `owner` and `repositories`
...
|-------|-----------|-------|
| Push tag to app repo | `apx-publish.yml` | App repo |
...
### Publish step fails with "permission denied"
...
- [Publishing Workflow](publishing-workflow.md) — manual vs CI publishing paths
