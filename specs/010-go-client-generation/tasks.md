...
- [X] **T010-1 [C]** Add optional `Builder` and `Publisher` interfaces to
  ...
  change for angular.
- [X] **T010-2 [C]** New `internal/client/goclient.go`: `goGenerator` implements `Generator`
...
- [X] **T010-3 [S]** `--package`→Go-module-path handling for `--generator go` (spec resolution in
  `client.go`); derive a default module path when empty; `--scope` ignored for go. Doc the rule.
- [X] **T010-4 [C]** goGenerator implements `Builder` (`go build ./...` in OutputDir; error ⇒
...
- [X] **T010-5 [S]** Enriched-OpenAPI fixture under apx `testdata/` (a toy spec carrying
...
- [X] **T010-6 [C]** e2e test: generate from the fixture with the real tool, run the go
...
- [X] **T010-7 [S]** Backward-compat test: angular still registered; angular build/publish routes
  to npm (Builder/Publisher not implemented by angular ⇒ fallback). Existing
  `internal/client/client_test.go` green (AC-5/FM-4).
- [X] **T010-8 [S]** `apx client publish --generator go --dry-run` records `go-module` artifact,
  no token needed (AC-4). Command help lists `go` (AC-6).
- [X] **T010-9 [S]** devedge thin driver (FR-8): extend `de api publish --client` to pass
  `--generator go`. (Separate repo `/Users/dgarcia/go/src/github.com/infobloxopen/devedge`; keep
  minimal — do LAST, after apx is green.)
- [X] **T010-10 [S]** `go build ./... && <apx test target>` green; `apx` help lists `go`; update
